...
final class WebExtensionManager: NSObject, WebExtensionManaging {
    ...
    private func makeContext(for webExtension: _WKWebExtension) -> WKWebExtensionContext {
    ...
    lazy var extensions: [_WKWebExtension] = {
        // Bundled extensions
        let emoji = loadWebExtension(path: Bundle.main.path(forResource: "emoji-substitution", ofType: nil)!)
...
//        let nightEye = WebExtensionManager.loadWebExtension(path: "/Applications/Night Eye.app/Contents/PlugIns/Night Eye Extension.appex/Contents/Resources/")
    ...
    func toolbarButtons() -> [NSButton] {
        return contexts.enumerated().map { (index, context) in
            ...
            let button = NSButton(image: image, target: self, action: #selector(WebExtensionManager.toolbarButtonClicked))
            button.tag = index
            return button
        }
    }
